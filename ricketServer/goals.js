...
router.post('/', async (req, res) => {
  ...
  db.pool.query(q).then(result => {res.json({ goals: result.rows, error: false }) })
                  .catch(result => {res.json({ goals: result, error: "insert failed." })})
})
...
router.get('/:goalId', async (req, res) => {
  ...
  db.pool.query(q).then(result => {res.json({ goals: result.rows, error: false })
  }).catch(result => { res.json({ goals: result, error: "no rows! is the db empty, did the query not match or is there another error?" }) })
})
...
router.put('/:goalId', async (req, res) => {
  ...
  // returning hard!!
  db.pool.query(q).then(result => {res.json({ goals: result.rows, error: false })
  }).catch(result => {res.json({ goals: result, error: "some error occured." }) })
})
